...
export class SponsorsPage {
		...
		this.databaseprovider.getSponsorData(flags, "0").then(data => {
			...
			if (data['length']>0) {
				...
				for (var i = 0; i < data['length']; i++) {
					...
					if (imageAvatar != "") {
						imageAvatar = "https://discus.convergence-us.com/AdminGateway/images/Sponsors/" + imageAvatar;
    ...
    navToEmail(EmailAddress) {
        if (EmailAddress === undefined) {
            ...
            window.open('mailto:' + EmailAddress + '?subject=DISCUS Fly-in Planner','target=_blank');
        }
