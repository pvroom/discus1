...
export class LoginPage {
	...
	LoginUser() {
		...
		if (this.LoginName == undefined || this.LoginPassword == undefined) {
			...
			var URL = 'https://discus.convergence-us.com/flyinPlanner.php';
			...
			this.http.get(URL).map(res => res.json()).subscribe(
				data => {
					...
					console.log("Attendee LoginType: " + data.LoginType);
					...
					console.log("Flyin Meeting Days: " + data.FlyinMeetingDays);
					...
					this.localstorage.setLocalValue('LoginType', data.LoginType);
					...
					this.localstorage.setLocalValue("AgendaDays", data.FlyinMeetingDays);
					...
					this.localstorage.setLocalValue('CallPrimeNumber', data.CallPrimeNumber);
					this.localstorage.setLocalValue('TextPrimeNumber', data.TextPrimeNumber);
					this.localstorage.setLocalValue('FlyinType', data.FlyinType);
					...
					if (data.status=="200") {
						...
						var DefaultUserSyncStart = this.localstorage.getLocalValue('UserSyncStart');
						var LastSync3 = this.localstorage.getLocalValue('LastUserSync');
						if (LastSync3 == '' || LastSync3 === null) {
							LastSync3 = DefaultUserSyncStart;
						}
						...
						console.log('Login: User Sync period: ' + LastSync + ' to ' + ThisSync);
				...
				err => {
					...
					switch(err.status) {
						...
						case 500:
							...
							break;
						default:
							ErrorMessage = "We're sorry. The credentials entered could not be validated for DISCUS. Possible reasons include:<br/> 1) You're not using the email address submitted to DISCUS;<br/> 2) You've mistyped your email address or password.";
